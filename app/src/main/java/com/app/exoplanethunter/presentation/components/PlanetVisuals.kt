...
fun planetTypeLabel(planet: Exoplanet): String =
    planetTypeLabel(planet.planetRadiusEarth, planet.planetMassEarth)

/** Field-based overload so non-domain callers (e.g. the widget) can reuse the same logic. */
fun planetTypeLabel(radiusEarth: Double?, massEarth: Double?): String {
    val r = radiusEarth
    val m = massEarth
    return when {
        r != null -> when {
...
fun planetAccentColor(planet: Exoplanet): Color =
    planetAccentColor(planet.equilibriumTempK, planet.planetRadiusEarth, planet.planetMassEarth)

/** Field-based overload so non-domain callers (e.g. the widget) can reuse the same logic. */
fun planetAccentColor(equilibriumTempK: Double?, radiusEarth: Double?, massEarth: Double?): Color {
    val t = equilibriumTempK
    return when {
        ...
        t != null -> ScorchingRed
        else -> planetTypeColor(planetTypeLabel(radiusEarth, massEarth))
    }
}
